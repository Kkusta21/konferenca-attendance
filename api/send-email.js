...
export default async function handler(req, res) {
  ...
  const html = `<div style="font-family:'Segoe UI',sans-serif;max-width:520px;margin:0 auto;background:#0a0a0f;color:#e8e8f0;border-radius:12px;overflow:hidden"><div style="background:linear-gradient(135deg,#00e5b4,#00bcd4);padding:32px;text-align:center"><h1 style="margin:0;font-size:1.6rem;color:#000">🎫 Karta Juaj e Hyrjes</h1><p style="margin:8px 0 0;color:#000;opacity:0.7">${SENDER_NAME}</p></div><div style="padding:32px"><p style="font-size:1rem;margin:0 0 8px">Përshëndetje, <strong>${name}</strong>!</p><p style="color:#9999bb;font-size:0.88rem;margin:0 0 24px">Ky kod QR është kartela juaj e hyrjes për të gjitha eventet e konferencës.</p><div style="text-align:center;margin-bottom:24px"><img src="${qrImageUrl}" style="width:220px;height:220px;border-radius:8px;border:8px solid #fff"/></div><div style="background:#1c1c28;border:1px solid #00e5b4;border-radius:6px;padding:12px;text-align:center;margin-bottom:24px"><p style="margin:0;font-size:0.72rem;color:#9999bb;text-transform:uppercase;letter-spacing:0.1em">Kodi juaj unik</p><p style="margin:6px 0 0;font-family:monospace;font-size:1.3rem;letter-spacing:0.15em;color:#00e5b4">${code}</p></div><p style="color:#6b6b8a;font-size:0.75rem;text-align:center;margin:0">⚠️ Ky kod është personal. Mos e ndani me të tjerët.</p></div></div>`;
